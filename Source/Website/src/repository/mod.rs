...
use tantivy::{Index, query, Score, Term};
...
impl Repository {
    pub async fn from_url(index_url: &str, raw_url: String) -> Repository {
        ...
        let package_meta_cache = CacheBuilder::new(10_000).time_to_live(std::time::Duration::new(30, 0)).build();
        let package_cache = CacheBuilder::new(2_000).time_to_live(std::time::Duration::new(30, 0)).build();
    ...
    pub async fn get_package_index_data_by_id(&self, id: String) -> Option<Vec<semver::Version>> {
        ...
        tokio::task::spawn_blocking(move || {
            let query = query::BooleanQuery::new(vec![(Occur::Must, Box::new(query::TermQuery::new(
            ...
            if let [(_, address)] = searcher.search(&query, &TopDocs::with_limit(1)).ok()?[..] {
                ...
                Some(doc.get_all(package_schema.versions).map(|v| semver::Version::parse(v.as_str()?).ok()).flatten().collect())
            } else {
                None
            }
        }).await.ok().flatten()
    ...
    pub async fn get_version_meta(&self, id: &str, version: &str) -> Result<metadata::Version, StatusCode> {
        let s = read_file_or_url_as_string(&self.path(&format!("/Packages/{id}/v{version}/metadata.toml")))
            .await.ok_or(StatusCode::NOT_FOUND)?;
        toml::from_str(&s).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
    }

    pub async fn get_package_meta_by_id(&self, id: &str) -> Result<metadata::Package, StatusCode> {
        self.package_meta_cache.try_get_with_by_ref(id, async {
            let s = read_file_or_url_as_string(&self.path(&format!("/Packages/{id}/metadata.toml")))
                .await.ok_or(StatusCode::NOT_FOUND)?;
            toml::from_str(&s).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)
        }).await.map_err(|e| *e)
    }
    ...
    pub async fn get_package_readme_by_id(&self, id: &str) -> Option<model::Readme> {
        if let Some(readme) = read_file_or_url_as_string(&self.path(&format!("/Packages/{id}/README.md"))).await {
            Some(Readme::Markdown(readme))
        } else {
             read_file_or_url_as_string(&self.path(&format!("/Packages/{id}/README.adoc"))).await.map(|s| {
                 Readme::ASCIIDOC(s)
             })
        }
    }
    ...
    pub async fn get_package_by_id(&self, id: &str) -> Result<model::Package, StatusCode> {
        self.package_cache.try_get_with_by_ref(id, async {
            let metadata = async { self.get_package_meta_by_id(id).await };
            let readme = async { self.get_package_readme_by_id(id).await.ok_or(StatusCode::NOT_FOUND) };
            let versions = async {
                try_join_all(self.get_package_index_data_by_id(id.to_string())
                    .await.ok_or(StatusCode::NOT_FOUND)?
                    .into_iter().map(|version| async {
                        let metadata = self.get_version_meta(id, &version.to_string()).await?;
                        Ok(model::Version{
                            version,
                            fin_version: metadata.fin_version,
                            game_version: metadata.game_version,
                            mod_dependencies: metadata.mod_dependencies.into_iter().map(|m| model::ModDependency {
                                id: m.id,
                                version: m.version,
                            }).collect(),
                            eeprom: metadata.eeprom.into_iter().map(|e| model::EEPROM {
                                name: e.name,
                                title: e.title,
            ...
            Ok(model::Package::from_metadata(id.to_string(), readme, versions, metadata))
        }).await.map_err(|e| *e)
