...
struct SerializeSemverReq<'a>(
	#[serde(serialize_with = "serialize_semver_req")] &'a semver::VersionReq,
);

pub fn serialize_semver_req<S: Serializer>(
	v: &semver::VersionReq,
	s: S,
) -> Result<S::Ok, S::Error> {
	s.serialize_str(&v.to_string())
}
pub fn serialize_semver_req_opt<S: Serializer>(
	v: &Option<semver::VersionReq>,
	s: S,
) -> Result<S::Ok, S::Error> {
	match v {
...
pub fn deserialize_semver_req<'d, D: Deserializer<'d>>(
	d: D,
) -> Result<semver::VersionReq, D::Error> {
	d.deserialize_str(SerializeSemverReqVisitor)
}
pub fn deserialize_semver_req_opt<'d, D: Deserializer<'d>>(
	d: D,
