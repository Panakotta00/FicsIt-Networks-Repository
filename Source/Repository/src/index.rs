...
pub fn build_schema() -> (Schema, PackageSchema) {
    ...
    let package = PackageSchema {
        ...
        version_data: builder.add_bytes_field("version_data", FAST),
    };
...
pub fn add_package_to_index(index_writer: &IndexWriter<TantivyDocument>, package_schema: &PackageSchema, package: Package) -> tantivy::Result<tantivy::Opstamp> {
    ...
    for version in package.versions {
        let version_data = VersionData{
            ...
            mod_dependencies: version.mod_dependencies.into_iter().map(|m| ModDependency{
                id: m.id.clone(),
